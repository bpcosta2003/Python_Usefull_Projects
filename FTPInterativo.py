...
while menu == "1" or menu == "2" or menu == "3":
    menu = input(
        "Escolha a opção desejada: <1> - para Listar arquivos <2> - para definir um diretório <3> - para baixar um arquivo: ")
    if menu == "1":
        print(ftp.dir())
    elif menu == "2":
        ...
        tipo = input(
            "Digite <B> para arquivo binário ou qualquer outra letra para arquivo ASCII: ").upper()
        if tipo == "B":
            with open(input("Digite o nome do arquivo destino: "), 'wb') as arq:
                ftp.retrbinary(
                    'RETR ' + input("Arquivo de origem: "), arq.write)
        else:
            with open(input("Digite o nome do arquivo destino: "), 'w') as arq:
                ...
                ftp.retrlines(
                    'RETR ' + input("Arquivo de origem:"), escreverLinha)
